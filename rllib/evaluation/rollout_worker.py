    ...
    def __init__(
        ...
        if isinstance(self.env, (BaseEnv, MultiAgentEnv)):
            ...
            def wrap(env):
                ...
                return env
# >>>>>>> f4d41c408 (necessary changes for deployment of a rectangular (height!=width) observation by the deepmind-atari-wrapper. changes: wrap_rectangular_deepmind and WarpRectangularFrame. Important: Just taken if the keys 'dim_height' and 'dim_width' exists in config['model']['custom_model_config'])
        else:
            self.env = _validate_env(env_creator(env_context))
